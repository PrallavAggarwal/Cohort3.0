...
function isAdult(users: User[]): void {
  for (let i = 0; i < users.length; i++) {
    console.log(users[i])
    console.log(users[i]?.name) //added ?. so that if null then not cause error.
  }
}

let users: User[] = [{ name: "lee", age: 23 }, { name: "ben", age: 34 }]
...
type carProfile = Pick<Vehicles, "cars">;
// interface carProfile = Pick<Vehicles, "cars">;

const displayCarProfile = (profile: carProfile): void => {
  console.log(`Brand of car : ${profile.cars.brand} and price is ${profile.cars.price}`);
}

let car: carProfile = { cars: { brand: "kia", price: 1000000 } }
displayCarProfile(car)
